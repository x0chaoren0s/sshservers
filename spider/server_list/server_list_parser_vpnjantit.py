...
class Server_list_parser_vpnjantit(Server_list_parser_base):
    name = 'Server_list_parser_vpnjantit'

    def __init__(self, server_list_url: str = 'https://www.vpnjantit.com/free-ssh-7-days',
    ...
    def parse(self) -> dict:
        ...
        for region,host,ip_url,url in tqdm(zip(region_str_list,server_host_list,server_ip_url_list,server_url_list),
            ...
            if not self.check_server(ip):
                continue
            ret[url] = {'region': region, 'host': host, 'ip': ip, 'port': 22, 'Referer': res.url}
        return ret
